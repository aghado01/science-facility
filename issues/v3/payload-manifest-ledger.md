...
| 6 | **Strip tier** — semantics-preserving vs semantics-altering | tier 2 means the payload no longer runs and is a *redacted* artifact; a reader that assumes otherwise draws wrong conclusions. Needs a header-level summary; #5 is the per-entry evidence | open | comment-ontology §"Beyond comments — excisable code regions" |

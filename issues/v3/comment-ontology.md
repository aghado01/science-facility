...
| `region-markers` | `#region`/`#endregion`, `//region` | strip by default (rendering channel — editor folding furniture, compiler-ignored) |
| *(future)* commented-out code | comment spans that parse as code | candidate for debias mode |

Not comments — but *not uniformly protected either* (corrected 2026-08-04, the
table above and this line used to contradict each other on `#region`):

- **Never strip** — C# `#if`/`#elif`/`#else`/`#endif` (conditional compilation:
  removing them changes what compiles); PS `using` statements.
- **Keep by default** — C# `#pragma` (compiler reads it; warning suppression).
- **Strip by default** — `#region`/`#endregion` in every language that has them.
  Syntactic in C#, comment-lexed in PowerShell, and in both cases **ignored by
  the compiler**: pure editor-folding furniture, i.e. the rendering channel.

### Partition mechanism and default policy are independent (user, 2026-08-04)

`#region` is ontologically distinct from a `# comment` and should be recognized
as its own kind — mechanically the SAME treatment as `#Requires` (demultiplexed
out of the comment population at the parse boundary, promoted to a typed kind,
zero text predicates downstream). But its default policy is the opposite:
**strip, like ordinary comment blobs.**

That separation is the general rule. The partition exists to NAME the channel,
not to protect it. Once a kind is named, its policy is a per-kind decision:

| kind | partitioned? | default |
|---|---|---|
| `frontmatter` (`#Requires`) | yes | never strip |
| `region-markers` (`#region`) | yes | strip |
| `signature-blocks` (`# SIG #`) | yes (open item) | strip aggressively |

**PowerShell multiplexes three channels onto `#`** — documentation (ordinary
comments), runtime directive (`#Requires`), and rendering (`#region`) — which is
the sharpest statement of why the `#` overload costs so much here. Demultiplexing
is unavoidable; what varies afterward is only policy.

**The recognition mechanism is language-specific, which the shared kind name
hides** (user, 2026-08-04):

- **PowerShell** — `#region` lexes as `TokenKind.Comment`. It becomes a *derived*
  comment token by virtue of being addressed in the context of comment stripping,
  not by being commentary. Demultiplexing at the parse boundary is required, same
  machinery as `#Requires`.
- **C#** — comments are `//` and `/* */` in several varieties; `#region` is a
  directive (Roslyn: `RegionDirectiveTriviaSyntax` — structured trivia, not
  comment trivia). It never enters the comment population, so a comment stripper
  has nothing to demultiplex and cannot reach it. Removing it there is a separate,
  directive-level concern.

So `region-markers` is one ontology kind reached by two different mechanisms —
the architecture principle at the top of this doc, in miniature. The "partitioned?"
column above is a PowerShell answer; in C# the question does not arise.
...
4b. **SUSPECTED BUG — C# comment stripping may not be working** (user observation,
   2026-08-04, from a recent C# ingestion). **Observed against LTS**, which is
   still the production path while v3 is not off the ground. Unknown whether the
   fault is the stripper itself, the dispatch/handling around it, or normalization
   ordering. Investigate LTS first, then check whether v3 `rs-csstrip` inherited
   it — C# stripping was implemented in LTS first and ported to v3.
   Already known, to narrow the search:
   - v3 `rs-csstrip` passes a 73-assert suite over a synthetic fixture covering all
     six kinds (added 2026-08-04), so it is not globally inert. That points the LTS
     fault at LTS's own implementation or its dispatch — or at input shapes the
     fixture does not cover.
   - `rs-csstrip` is **regex-only**: no C# AST is reachable from PowerShell, so
     there is no AST-first route and nothing to fall back *to*. The documented
     limitation is string-literal false positives (`//` or `/*` inside `"..."`,
     verbatim `@"..."`, interpolated `$"..."`, raw `"""`), which would
     **over**-strip.
   - **Under**-stripping (nothing removed) points elsewhere entirely — ops
     resolution, dispatch, or the stripper never being reached. Establishing which
     direction the failure runs is the first discriminator.

...
**Artifacts:**
- [issues/v3/comment-ontology.md](../reposnapshot/design/comment-ontology.md) — design note: the kind table (frontmatter/directive, doc-strings, block/comment-block/line/inline, signature-blocks, region-markers, future commented-out-code) with per-kind default policies, mode presets (economy / debias / faithful), per-language directive lists (PS, C#, Python, JS/TS), the five action items, and the under-strip-never-corrupt invariant.
- The [LTS corruption brief](../reposnapshot/reports/lts-stripcomments-string-corruption.md) got a pointer plus the two PS-specific requirements (frontmatter preservation in the token path, `# SIG #` blocks as a distinct strippable kind — that one's a large token win since signature blobs are pages of base64), so the running chip session inherits the language-specific framing and won't extend the `Normalize-FileContent` extension-switch.
...
1. **Signature-blocks rationale** ([comment-ontology.md](../reposnapshot/design/comment-ontology.md) item 2) — I'd framed it as "add so they get stripped," but SIG runs already fall to `comment-blocks` under default ops in both strippers. The kind's real purpose is taxonomy and independent control (a keep-comments profile that still drops signatures), so it's reworded that way.
...
Corrected in the [format note](../reposnapshot/design/shard-format-notes.md) and memory. The format description now reads with the roles properly separated:
...
[shard-format-notes.md](../reposnapshot/design/shard-format-notes.md) and memory now carry the full picture from the new snapshot:
...
Recorded in the [audit](../reposnapshot/reports/lts-v3-transfer-audit.md) (as the spine-replacement workstream) and memory. Two syntheses fall out of this that are worth having said out loud:
...
Captured in [issues/mcp-surface.md](../reposnapshot/design/mcp-surface.md) and memory. A few observations worth adding to the vision as stated:
...
Everything from today is sitting in the six briefs ([lts-stripcomments](../reposnapshot/reports/lts-stripcomments-string-corruption.md), [comment-ontology](../reposnapshot/design/comment-ontology.md), [lts-v3-transfer-audit](../reposnapshot/reports/lts-v3-transfer-audit.md), [shard-format-notes](../reposnapshot/design/shard-format-notes.md), [thread-corpus-container](../reposnapshot/design/thread-corpus-container.md), [mcp-surface](../reposnapshot/design/mcp-surface.md)), the code changes are tested green (rs-psstrip 68/68, LTS 9/9 from the chip), and memory carries the doctrines. Ready for a first commit whenever you decide where the repo boundary goes.

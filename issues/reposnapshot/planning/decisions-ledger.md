...
| 31 | **Stage-appended attributes are the mechanism**, with `Encoding` from ingest as the first real customer | Recovers a pending update that dropped out of view (it lives in admiral-orchestration, not the consolidation plan). Building `Encoding` as a bespoke field would solve the general problem badly, once. Forces admiral residue #2 — ownership on append — to be settled rather than left latent; recommend copy-on-append, the fleet's existing posture | filed | stage-appended-attributes-brief |

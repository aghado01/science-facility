...
| P12 | **AGY here means the *application*** — Google Antigravity CLI `agy.exe`, adapter `agy/unverified-v1`, `transport.kind: "unsupported"` — and **not** the adjutant *role* named `agy` that the prototype drove with headless `claude.exe`. The application remains fail-closed until its profile is independently evidenced by a fresh native stream capture. The stale architecture report is not evidence of AGY correctness | ruled; standing. **Amended 2026-08-19** — the one capture attempt (2026-08-14) never reached a stream: `agy 1.1.13` exited at Google OAuth timeout after 60s with **0 bytes of stdout**. So the blocker in front of P12 is an *authentication-carrier* gap, not a stream-correctness finding, and nothing is yet known either way about this application's stream. The productive 2026-08-10→13 engagements in [agy-usage-report](../specimens/agy-usage-report-20260813_100903.md) were claude-backed and evidence the *role*, not this application. Evidence: [agy-native-stream-capture-20260814](../specimens/agy-native-stream-capture-20260814/README.md) |
...
| P22 | **`verification.status` carries two independent facts.** `allOf[0]` binds them: `verified` ⇒ full mappings + live transport + `evidence`; `else` ⇒ `reason` required, `capabilities` const all-false, `transport.kind` const `unsupported`, mapping blocks forbidden. A profile is therefore either fully specified *and* evidenced, or wholly inert. The two facts — **is it fully specified** and **has it been checked against a real client** — travel together for real adapters, which is why the binary works. `fake-client.json` is the counterexample that separates them: fully specified because it must be to exercise registry resolution, unchecked because nothing real exists to check it against. It cites *itself* as evidence for that reason. Mocking is not the escape — the test it breaks is precisely "strict provider and fake adapter fixtures load together", so stubbing the validator deletes the subject | **OPEN** — found 2026-08-19 cleaning the fakes; self-citation carries a `note` so it reads as a forced move. Same shape as the `reference` morphism question and **should be decided in the same pass**, or the two answers risk disagreeing. Candidate split: `specification: complete\|inert` × `checked: yes\|no`, registry resolving on the first, P12-style gating reading the second |

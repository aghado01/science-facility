...
| P22 | **The adapter schema cannot say "test double".** `allOf[0]` makes `verification.status: "verified"` require `correlation_mappings`, `provenance_mappings`, `prompt_delivery`, `terminal_events`, `record_mappings`, `verified_versions`, and `evidence` — right for a real client. But the registry resolves verified adapters only, so a fixture adapter that exists purely for registry resolution must claim `verified` and is then forced to cite evidence that cannot exist. `fake-client.json` cited *itself* for exactly this reason. Candidate fix: a third `status` value (`fixture`/`synthetic`) that the registry resolves but the evidence coupling does not apply to | **OPEN** — found 2026-08-19 while cleaning the fakes; the self-citation now carries a `note` saying why, so it reads as a forced move rather than drift |

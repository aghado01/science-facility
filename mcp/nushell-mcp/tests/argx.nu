...
let results = [
    ...
    (t "unknown flags never bind to _pos" {
        let p = ("probe --bogus 1 --nope=v 2 3" | argx parse)
        assert-eq $p._args [probe --bogus "1" --nope=v "2" "3"] "all kept in _args"
        assert-eq $p._pos.a "1" "a skips --bogus"
        assert-eq $p._pos.b "2" "b skips --nope=v"
        assert-eq $p._pos.x "3" "x"
        assert-eq $p._pos.y [] "no flag leaks into rest"
    })
    (t "tokens after -- still bind to _pos" {
        let p = ("probe 1 -- -g 3" | argx parse)
        assert-eq $p._pos.a "1" "a"
        assert-eq $p._pos.b "-g" "b is literal -g"
        assert-eq $p._pos.x "3" "x"
    })
]

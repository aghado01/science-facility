...
- **Builtin `inspect` is a passthrough, not a census.** It prints a debug table to the terminal — which does not exist under `--mcp` — and returns its input *unchanged*, so `$big | inspect` dumps the whole value into the tool result. It is the flood trap wearing the name you would reach for. For "what is this" use `dataspection`'s `shape` (see `nu-skills read dataspection`); the builtin is left unshadowed on purpose (see `nu-skills read appendix/inspect`).
- **`schema` (dataspection) shadows SQLite's `schema`** while the module is in the overlay. Ours profiles a population (`$x | schema`); the builtin shows a SQLite database schema. Prefer `nu-skills read dataspection` for the drill loop.

...
- **Builtin `inspect` is a passthrough, not a census.** It prints a debug table to the terminal — which does not exist under `--mcp` — and returns its input *unchanged*, so `$big | inspect` dumps the whole value into the tool result. It is the flood trap wearing the name you would reach for. For "what is this" use the `inspect` module's `shape`; the builtin is left unshadowed on purpose.

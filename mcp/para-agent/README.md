...
**Verified AGY mediation.** AGY is present only as a fail-closed, unverified profile. A native-stream probe against installed AGY 1.1.13 on 2026-08-14 produced zero stream bytes and surfaced an authentication-wait timeout, but operator diagnosis now points to an AGY launch/configuration problem — possibly specific to launching through Nu — rather than an authentication failure. The timeout is therefore a symptom, not the root-cause classification, and no fixture or capability was inferred from it. AGY must not be used for `delegate` until the launch issue is resolved and a current version-labelled native stream passes the common adapter and real-client conformance gates. Transcript or SQLite metadata is not an acceptable model-identity fallback.
